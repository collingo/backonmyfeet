...
function JustGivingDonations() {
	return $.getJSON('https://api.justgiving.com/fbfeb0e5/v1/fundraising/pages/backonmyfeet/donations?format=json&callback=?');
}

// Just Giving Total
function JustGivingTotal() {
	return $.getJSON('https://api.justgiving.com/fbfeb0e5/v1/fundraising/pages/backonmyfeet?format=json&callback=?');
}
...
$(function() {
	...
	$.when( VirginTotal(), JustGivingTotal() ).then(function( virginTotal, justGivingTotal ){
		...
		var grandTotal = parseInt(virginTotal[0].money_total, 10) + parseInt(justGivingTotal[0].grandTotalRaisedExcludingGiftAid, 10);

		$("#steps").text(grandTotal);
		$("#steps-label").css("display", "inline");
	...
	$.when( VirginDonations(), JustGivingDonations() ).then(function( virgin, justGiving){
		...
		// add virgin donations to allDonations
		$.each(justGivingDonations, function(index, donation) {
			allDonations.push({
		...
		$.each(allDonations, function(index, value) {
			...
			if(value.message != "") {
				add_marker(henleyCoords[extra].lat, henleyCoords[extra].long, "<p style=\"color: #d9593d; font-size: 13px; text-transform: uppercase\">"+value.message+"<br><br>"+value.name+"<br>Donated "+value.amount+" steps</p>"+share);
			} else {
				add_marker(henleyCoords[extra].lat, henleyCoords[extra].long, "<p style=\"color: #d9593d; font-size: 13px; text-transform: uppercase\">"+value.name+"<br>Donated "+value.amount+" steps</p>"+share);
			}
		});
			...
			setTimeout(function(){
				...
				$.each(allDonations, function(index, value) {
					...
					if(sp.match(si)) {
						...
						if(value.message != "") {
							var pp = "<p style=\"font-size: 13px; text-transform: uppercase\">"+value.message+"<br><br>"+value.name+"<br>Donated "+value.amount+" steps"+share+"</p>";
						} else {
							var pp = "<p style=\"font-size: 13px; text-transform: uppercase\">"+value.name+"<br>Donated "+value.amount.toFixed(2)+" steps"+share+"</p>";
						}
